...
export function Header({ title, children, className }: HeaderProps) {
  ...
  return (
    <div
      className={twMerge(
        "py-6 px-5 h-[76px] flex items-center justify-between text-xl font-bold border-b border-b-grayBorder bg-white/75 sticky top-0 backdrop-blur-md z-10 dark:border-b-grayBorderDark dark:text-tweetColor dark:bg-bodyDark/60 sm:py-3",
        className
      )}
    >
      {children ? (
